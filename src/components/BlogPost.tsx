    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      {posts.map((post) => (
        ...
        <div key={post.title} className='glass-card hover-lift rounded-lg p-6 flex flex-col gap-4'>
          <Link to={`/post/${post.slug}`}> <img src={post.imagecover} alt={post.title} className='rounded-md shadow-md'/>
          ...
          <div className="flex items-center space-x-4"> <img src={post.authorAvatar} alt={post.author} className="w-12 h-12 p-1 rounded-full mr-2 shadow-md" /> 
